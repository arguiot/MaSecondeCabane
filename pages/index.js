...
function Home({ products, router, photos, t }) {
    ...
    React.useEffect(() => {
        ...
        const possibles = products.filter(e => (e.waitingForCollect != true && e.quantity >= 1 && e.favorite == true))
        if (possibles.length >= 12) {
            while (productSet.size < 12) {
                const p = shuffle(possibles)[0]
                productSet.add(p)
            }
        }
...
export async function getStaticProps({ locale }) {
    let photos = [];
    try {
        const response = await fetch('https://api.unsplash.com/collections/27372549/photos?per_page=10&order_by=latest&page=1', {
            headers: {
